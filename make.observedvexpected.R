...
if(!exists("census_84_allnucleot")){
  census_84_allnucleot<- fread("/Users/browna6/mahmood/binom/analyzed\ data/rebuilt_census_table_withmutabilities.csv")
...
mutability_cdf <- ggplot(all_census,aes(x = mutability,color = Measure)) + stat_ecdf(size = 2) + 
  ...
  labs(color = "Mutability Spectrum", x = bquote(bold('Mutability *'~10^6)),y = bquote(bold("Cumulative Probability"))) + 
